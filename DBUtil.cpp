...
DBUtil::~DBUtil() { CloseDB(); }

//打开数据库
bool DBUtil::OpenDB() {
  mysql_init(&myCont);
  if (mysql_real_connect(&myCont, szHost.c_str(), szUsername.c_str(),
                         szPswd.c_str(), szDatabase.c_str(), nPort, NULL, 0)) {
    isOpen = true;
    printf("打开db成功\n");
    return true;
  }
...
User DBUtil::ReadUser(string strUserName, string strUserPWD) {
  ...
  if (isOpen) {
    ...
    sql += "select * from user where name='" + strUserName +
           "' and password='" + strUserPWD + "'";
    printf("查询语句：%s\n", sql.c_str());
...
bool DBUtil::AddBorrowRecord(BorrowRecord borrowRecord) {
  ...
  if (isOpen) {
    sql = sql + "insert into borrowrecord values(null," + szBookId + "," +
          szUserId + ",'" + borrowRecord.m_tBorrowDate + "','" +
...
//查询某位用户的借阅记录
bool DBUtil::SelectBorrowRecordByUserId(vector<BorrowRecord> &borrowRecords,
...
bool DBUtil::FinishBorrowRecord(int nRecordId, int nBookId) {
  ...
  if (isOpen) {
    sql = sql + "update borrowrecord set returndate='" + szTime +
          "' where id=" + szRecordId;
    mysql_query(&myCont, sql.c_str());
...
bool DBUtil::AddUser(User user) {
  ...
  if (isOpen) {
    sql += "insert into user values(null,'" + user.m_strName + "','" +
           user.m_strPassword + "'," + szRoleType + ")";
    mysql_query(&myCont, sql.c_str());
  } else {
    cout << "connect failed!" << endl;
  }
...
bool DBUtil::ExtendBorrowRecord(int nRecordId) {
  ...
  time_t tTemp = time(NULL);
  if (isOpen) {
    sql = sql + "update borrowrecord set shouldreturndate='" +
          timeUtil.AddMonth(tTemp) +
          "',borrowrecord.continue=borrowrecord.continue+1 where id=" +
          szRecordId;
    mysql_query(&myCont, sql.c_str());
  } else {
    cout << "connect failed!" << endl;
